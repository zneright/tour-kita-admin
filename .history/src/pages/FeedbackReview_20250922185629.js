...
const FeedbackReview = () => {
    ...
    const handleShowMore = () => {
        setDisplayCount(prev => Math.min(prev + 5, Math.min(filteredFeedback.length, maxDisplay)));
    };

    const handleClose = () => {
        setDisplayCount(0); // reset back to initial state
    };
                            ...
                            {filteredFeedback.length > 5 && (
                                <tr>
                                    <td colSpan="7" style={{ textAlign: 'center', padding: '10px' }}>
                                        {displayCount < filteredFeedback.length ? (
