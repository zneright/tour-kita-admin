    ...
    const feedbackForCurrentTable = () => {
        if (viewLevel === "table" && selectedDay) {
            return feedbackList.filter((f) => {
                const d = asDate(f.createdAt);
                if (!d || !isSameDay(d, selectedDay)) return false;

                // Respect active tab
                if (isFeatureTab && f.feedbackType !== "App Feedback") return false;
                if (isLocationTab && f.feedbackType !== "Location Feedback") return false;

                // Hide null/empty feature/location
                if (isFeatureTab && !f.feature) return false;
                if (isLocationTab && !f.location) return false;
                        ...
                        {viewLevel === "weekly" && selectedMonth && (
                            <>
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                    <h3>
                                        Weeks in{" "}
                                        {selectedMonth.year && selectedMonth.monthIndex !== null
                                            ? format(new Date(selectedMonth.year, selectedMonth.monthIndex, 1), "MMMM yyyy")
                                            : "Unknown"}
                                    </h3>
                                    <div>
                                        <button
                                ...
                                <div className="drill-row">
                                    {weeksForMonth(selectedMonth.year, selectedMonth.monthIndex).map((wObj, idx) => {
                                        const entries = entriesFor(wObj.start, wObj.end, activeTab);
                                        const avgValue = averageForEntries(entries);
                                        const { top, low } = topAndBottomForEntries(entries);
                                ...
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                    <h3>
                                        ...
                                        {selectedWeekRange.start
                                            ? format(selectedWeekRange.start, "MMM yyyy")
                                            : "Unknown"}
                                    </h3>
                                    <div>
                                        <button
