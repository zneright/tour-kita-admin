...
const AnalysisReport = () => {
    ...
    const ageGroupData = useMemo(() => {
        ...
        const ageCounts = {}; // { '20-29': {Q1: 2, Q2:1}, ... }

        filteredUsers.forEach(u => {
            if (u.age != null) {
                const rangeStart = Math.floor(u.age / 10) * 10;
                const rangeLabel = u.age >= 90 ? '90+' : `${rangeStart}-${rangeStart + 9}`;
                        ...
                        {filter === 'Monthly'
                            ...
                            : filter === 'Quarterly'
                                ? ['Q1', 'Q2', 'Q3', 'Q4'].map((q, idx) => (
                                    <Bar key={q} dataKey={q} stackId="a" fill={COLORS[idx % COLORS.length]} />
                                ))
                                : [...new Set(filteredUsers.map(u => moment(u.registeredDate).year()))]
                                    .sort()
                                    .map((y, idx) => (
