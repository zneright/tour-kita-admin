...
const FeedbackReview = () => {
    ...
    const calculateStats = (data, tabType) => {
        ...
        const averageForEntries = (entries) => {
            const valid = entries.filter(e => typeof e.rating === "number");
        ...
        const validRatings = filtered.filter(f => typeof f.rating === "number");
        const avg = validRatings.length
            ? validRatings.reduce((acc, cur) => acc + cur.rating, 0) / validRatings.length
            : null;
        setAverageRating(avg ? avg.toFixed(1) : "N/A");
    ...
    const avgPerKey = (items, tabType) => {
        ...
        items.forEach((it) => {
            ...
            if (!map[key]) map[key] = [];
            if (typeof it.rating === "number") map[key].push(it.rating);
        });
    ...
    const topAndBottomForEntries = (entries, tabType) => {
        const arr = avgPerKey(entries, tabType).filter(e => e.count > 0);
                                ...
                                <div className="drill-row">
                                    {years.length ? (
                                        years.map((y) => {
                                            ...
                                            const avgValue = averageForEntries(entries, activeTab);
                            ...
                            <>
                                ...
                                <div className="drill-row">
                                    {weeksForMonth(selectedMonth.year, selectedMonth.monthIndex).map((w) => {
                                        ...
                                        return (
                                            <div key={w.label} className="drill-group">
                                                <PeriodCard
                                                    title={w.label}
                                                    avg={avgValue ? avgValue.toFixed(2) : "—"}
                                                    count={entries.length}
                            ...
                            <>
                                ...
                                <div className="drill-row">
                                    {daysForWeekRange(selectedWeekRange.start, selectedWeekRange.end).map((dayObj) => {
                                        ...
                                        return (
                                            <div key={dayObj.label} className="drill-group">
                                                <PeriodCard
                                                    title={dayObj.label}
                                                    avg={avgValue ? avgValue.toFixed(2) : "—"}
                                                    count={entries.length}
