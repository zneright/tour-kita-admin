...
const AnalysisReport = () => {
    ...
    const filteredUsers = useMemo(() => {
        return users.filter(user => {
            ...
            if (filter === 'Yearly') return true; // Include all months
            return date.year() === parseInt(selectedYear.toString());
        });
    ...
    const userTypeData = useMemo(() => {
        ...
        const foreign = filteredUsers.filter(u => u.userType === 'foreign national').length;
        return [
            { name: 'Students', value: student },
            { name: 'Tourists', value: tourist },
            { name: 'Researchers', value: researcher },
            { name: 'Locals', value: local },
            { name: 'Foreign Nationals', value: foreign },
        ];
    }, [filteredUsers]);
