...
const AnalysisReport = () => {
    ...
    const genderDataByMonth = useMemo(() => {
        const group = {};
        filteredUsers.forEach(u => {
            const month = moment(u.registeredDate).format('MMM');
            if (!group[month]) group[month] = {};
            const gender = u.gender || 'Prefer Not to Say';
            group[month][gender] = (group[month][gender] || 0) + 1;
        });

        return Object.entries(group).map(([month, counts]) => ({ month, ...counts }));
    }, [filteredUsers]);
