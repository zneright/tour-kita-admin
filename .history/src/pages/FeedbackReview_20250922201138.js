...
const FeedbackReview = () => {
    ...
    const PeriodCard = ({ title, avg, count, onClick }) => (
        <div className="drill-card" onClick={() => handleSelect(item)}>
            );

    // ---- Final filtered table for selected day/week/month etc.
    // For 'table' view we show feedbackList filtered to selectedDay (if selectedDay set),
    // or to selectedWeekRange if user arrives at weekly table (we only implement day table per your spec).
    const feedbackForCurrentTable = () => {
        if (viewLevel === "table" && selectedDay) {
            // include all entries that fall on that day (00:00 to 23:59)
            ...
            <div className="dashboard-wrapper">
              ...
              -------------------------------------------------- */}
                        <div className="ratings-drilldown" style={{ marginTop: 20 }}>
                            ...
                            {viewLevel === "yearly" && (
                                <>
                                    ...
                                    <div className="drill-row">
                                        {years.length ? (
                                            years.map((y) => {
                                                // compute entries for whole year
                            ...
                            {/* WEEKLY (weeks INSIDE selected month) */}
