...
const FeedbackReview = () => {
    ...
    const avgPerKey = (items, type = "auto") => {
        // type: "feature", "location", or "auto" (decides based on tab)
        const map = {};
        items.forEach((it) => {
            const key =
                type === "feature"
    ...
    const topAndBottomForEntries = (entries) => {
        if (!entries.length) return { topLocation: [], bottomLocation: [], topFeature: [], bottomFeature: [] };

        const locArr = avgPerKey(entries, "location");
        const featArr = avgPerKey(entries, "feature");

        return {
            topLocation: locArr.slice(0, 3),
            bottomLocation: locArr.slice(-3),
            topFeature: featArr.slice(0, 3),
            bottomFeature: featArr.slice(-3),
        };
    };
                            ...
                            <>
                                ...
                                <div className="drill-row">
                                    {quartersForYear(selectedYear).map((qObj) => {
                                        const { topLocation, bottomLocation, topFeature, bottomFeature } = topAndBottomForEntries(qObj.entries);
                                        const avgTop = topLocation[0]?.avg ?? topFeature[0]?.avg ?? null;
                                        ...
                                        return (
                                            <div key={qObj.quarter} className="drill-group" style={{ minWidth: 220 }}>
                                                ...
                                                <div style={{ marginTop: 8 }}>
                                                    <small>Top Location: {topLocation[0]?.key ?? "—"}</small><br />
                                                    <small>Low Location: {bottomLocation[0]?.key ?? "—"}</small><br />
                                                    <small>Top Feature: {topFeature[0]?.key ?? "—"}</small><br />
                                                    <small>Low Feature: {bottomFeature[0]?.key ?? "—"}</small>
                                                </div>
                                            </div>
                                        );
                                    })}
