...
const AnalysisReport = () => {
    ...
    const getUserActivityData = useMemo(() => {
        ...
        periodKeys.forEach(pk => activity[pk] = 0);
        ...
        return periodKeys.map(pk => ({ name: pk, users: activity[pk] || 0 }));
        ...
        if (filter === 'Quarterly') {
            ...
            breakdown = months.map(m => {
                ...
                payload.forEach(entry => {
                    if (entry.payload[m]) monthData += entry.payload[m];
                });
                return `${m}: ${monthData}`;
            });
        }
    ...
    const ageGroupChartData = useMemo(() => {
        ...
        const data = periodKeys.map(pk => {
            ...
            filteredUsers.forEach(u => {
                ...
                if (filter === 'Monthly') period = date.format('MMM');
                else if (filter === 'Quarterly') period = `Q${Math.ceil((date.month() + 1) / 3)}`;
                else period = date.year().toString();
