...
const FeedbackReview = () => {
    ...
    const calculateStats = (data, tabType) => {
        ...
        const averageForEntries = (entries) => {
            const valid = entries.filter(e => typeof e.rating === "number" && !isNaN(e.rating));
        ...
        // Update avgPerKey
        const avgPerKey = (items, tabType = activeTab) => {
            const map = {};
            items.forEach((it) => {
                let key;
                if (tabType === "Feature Feedback") key = it.feature || "N/A";
                else if (tabType === "Location Feedback") key = it.location || "N/A";
                else key = it.feature || it.location || "N/A";

                if (typeof it.rating === "number" && !isNaN(it.rating)) {
                    if (!map[key]) map[key] = [];
                    map[key].push(it.rating);
                }
            });
    ...
    const avgPerKey = (items, tabType = activeTab) => {
        ...
        items.forEach((it) => {
            ...
            if (!map[key]) map[key] = [];
            if (it.rating != null) map[key].push(it.rating); // only valid ratings
        });
    ...
    const quartersForYear = (year) => {
        ...
        for (let q = 1; q <= 4; q++) {
            ...
            const entries = entriesFor(qStart, qEnd, activeTab, { start: yearStart, end: yearEnd });
            arr.push({
                quarter: q,
                label: `Q${q} ${year}`,
                start: qStart,
    ...
    const monthsForQuarter = (year, quarter) => {
        ...
        for (let m = 0; m < 3; m++) {
            ...
            const entries = entriesFor(mStart, mEnd, activeTab, { start: quarterStart, end: quarterEnd });
            months.push({
                year,
                monthIndex,
    ...
    const weeksForMonth = (year, monthIndex) => {
        ...
        const weeks = weekStarts.map((ws) => {
            ...
            const entries = entriesFor(start, end, activeTab, { start: mStart, end: mEnd });
            const label =
    ...
    const daysForWeekRange = (start, end) => {
        return eachDayOfInterval({ start, end }).map((d) => {
            ...
            const entries = entriesFor(dayStart, dayEnd, activeTab, { start, end }); // parent = week
            return {
                date: d,
                    ...
                    <div className="ratings-drilldown" style={{ marginTop: 20 }}>
                            ...
                            <>
                                ...
                                <div className="drill-row">
                                    {quartersForYear(selectedYear).map((qObj) => {
                                        const entries = entriesFor(qObj.start, qObj.end, activeTab);
                                        const avgValue = averageForEntries(entries);
                                        const { top, low } = topAndBottomForEntries(entries);
                        ...
                        {viewLevel === "monthly" && selectedYear && selectedQuarter && (
                            <>
                                ...
                                <button onClick={() => { setViewLevel("quarterly"); setSelectedMonth(null); }}>
                                    ← Back to Quarters
                                </button>
                                <div className="drill-row">
                                    {monthsForQuarter(selectedYear, selectedQuarter).map((mObj) => {
                                        const entries = entriesFor(mObj.start, mObj.end, activeTab);
                                        const avgValue = averageForEntries(entries);
                                        const { top, low } = topAndBottomForEntries(entries);
                        ...
                        {viewLevel === "weekly" && selectedMonth && (
                            <>
                                <h3>Weeks — {format(new Date(selectedMonth.year, selectedMonth.monthIndex, 1), "LLLL yyyy")}</h3>
                                <button onClick={() => { setViewLevel("monthly"); setSelectedWeekRange(null); }}>
                                    ← Back to Months
                                </button>
                                <div className="drill-row">
                                    {weeksForMonth(selectedMonth.year, selectedMonth.monthIndex).map((w) => {
                                        const entries = entriesFor(w.start, w.end, activeTab);
                                        const avgValue = averageForEntries(entries);
                                        const { top, low } = topAndBottomForEntries(entries);

                                        return (
                                            <div key={w.start.toISOString()} className="drill-group">
                                                <PeriodCard
                                                    title={w.label}
                                                    avg={avgValue}
                                                    count={entries.length}
                        ...
                        {viewLevel === "daily" && selectedWeekRange && (
                            <>
                                <h3>Days — {format(selectedWeekRange.start, "MMM d")} to {format(selectedWeekRange.end, "MMM d, yyyy")}</h3>
                                <button onClick={() => { setViewLevel("weekly"); setSelectedDay(null); }}>
                                    ← Back to Weeks
                                </button>
                                <div className="drill-row">
                                    {daysForWeekRange(selectedWeekRange.start, selectedWeekRange.end).map((d) => {
                                        const entries = entriesFor(d.start, d.end, activeTab);
                                        const avgValue = averageForEntries(entries);
                                        const { top, low } = topAndBottomForEntries(entries);

                                        return (
                                            <div key={d.date.toISOString()} className="drill-group">
                                                <PeriodCard
                                                    title={d.label}
                                                    avg={avgValue}
                                                    count={entries.length}
                                                    onClick={() => goToTableForDay(d)}
                                                />
                                                <small className="top">Top: {top}</small><br />
                                                <small className="low">Low: {low}</small>
                                            </div>
