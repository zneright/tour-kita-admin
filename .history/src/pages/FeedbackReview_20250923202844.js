...
const FeedbackReview = () => {
    ...
    const calculateStats = (data) => {
        if (!data || data.length === 0) {
            ...
            setMostLovedOverall("N/A");
            setAreaOfConcernOverall("N/A");
            setOverallLocationRating("N/A");
            setOverallFeatureRating("N/A");
            return;
        }
        ...
        // 🔹 Location Feedback
        const locationFeedback = data.filter((i) => i.feedbackType === "Location Feedback");
        const locationGroups = groupBy(locationFeedback, "locationName");

        let mostLovedLoc = null;
        let lowestLoc = null;
        let highestLocAvg = -1;
        let lowestLocAvg = 6;
        ...
        Object.keys(locationGroups).forEach((loc) => {
            const avg =
                locationGroups[loc].reduce((a, c) => a + (c.rating || 0), 0) /
                locationGroups[loc].length;

            if (avg > highestLocAvg) {
                highestLocAvg = avg;
                mostLovedLoc = loc;
            }
            if (avg < lowestLocAvg) {
        ...
        // Overall Location average
        const avgLocation =
            locationFeedback.reduce((a, c) => a + (c.rating || 0), 0) / locationFeedback.length;

        setMostLovedLocation(mostLovedLoc || "N/A");
        setAreaOfConcern(lowestLoc || "N/A");
        setOverallLocationRating(isNaN(avgLocation) ? "N/A" : avgLocation.toFixed(1));

        // 🔹 Feature Feedback
        const featureFeedback = data.filter((i) => i.feedbackType === "App Feedback");
        const featureGroups = groupBy(featureFeedback, "featureName");

        let mostLovedFeat = null;
        let lowestFeat = null;
        let highestFeatAvg = -1;
        let lowestFeatAvg = 6;

        Object.keys(featureGroups).forEach((feat) => {
            const avg =
                featureGroups[feat].reduce((a, c) => a + (c.rating || 0), 0) /
                featureGroups[feat].length;

            if (avg > highestFeatAvg) {
                highestFeatAvg = avg;
                mostLovedFeat = feat;
            }
            if (avg < lowestFeatAvg) {
                lowestFeatAvg = avg;
                lowestFeat = feat;
            }
        });

        // Overall Feature average
        const avgFeature =
            featureFeedback.reduce((a, c) => a + (c.rating || 0), 0) / featureFeedback.length;

        setMostLovedFeature(mostLovedFeat || "N/A");
        setAreaOfConcernFeature(lowestFeat || "N/A");
        setOverallFeatureRating(isNaN(avgFeature) ? "N/A" : avgFeature.toFixed(1));

        // 🔹 Overall (all feedback combined)
        const allGroups = groupBy(data, (item) =>
            item.feedbackType === "Location Feedback" ? item.locationName : item.featureName
        );

        let mostLovedAll = null;
        let lowestAll = null;
        let highestAllAvg = -1;
        let lowestAllAvg = 6;

        Object.keys(allGroups).forEach((name) => {
            const avg =
                allGroups[name].reduce((a, c) => a + (c.rating || 0), 0) /
                allGroups[name].length;

            if (avg > highestAllAvg) {
                highestAllAvg = avg;
                mostLovedAll = name;
            }
            if (avg < lowestAllAvg) {
        ...
        setMostLovedOverall(mostLovedAll || "N/A");
        setAreaOfConcernOverall(lowestAll || "N/A");
    };
    ...
    const avgPerKey = (items) => {
        const map = {};
        items.forEach((it) => {
            // Use feature or location dynamically
                            ...
                            <>
                                ...
                                <div className="drill-row">
                                    {quartersForYear(selectedYear).map((qObj) => {
                                        const { top, low } = topAndBottomForEntries(qObj.entries);
