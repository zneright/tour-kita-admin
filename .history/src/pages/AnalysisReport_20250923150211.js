    ...
    const getUserActivityData = useMemo(() => {
        ...
        filteredUsers.forEach(user => {
            ...
            if (filter === 'Monthly') {
                ...
                key = `Q${Math.ceil((date.month() + 1) / 3)}`; // Q1, Q2, ...
            } else if (filter === 'Yearly') {
                key = date.format('MMM D'); // Jan 9, Feb 15, ...
            }

            activity[key] = (activity[key] || 0) + 1;
        });

        // Generate all possible keys based on filter
        let allKeys = [];
        if (filter === 'Monthly') {
            allKeys = moment.monthsShort();
        } else if (filter === 'Quarterly') {
            allKeys = ['Q1', 'Q2', 'Q3', 'Q4'];
        } else if (filter === 'Yearly') {
            // generate all dates of the selected year
            for (let m = 0; m < 12; m++) {
                const daysInMonth = moment(`${selectedYear}-${m + 1}`, 'YYYY-M').daysInMonth();
                for (let d = 1; d <= daysInMonth; d++) {
                    allKeys.push(moment(`${selectedYear}-${m + 1}-${d}`, 'YYYY-M-D').format('MMM D'));
                }
            }
        }
