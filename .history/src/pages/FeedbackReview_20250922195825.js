...
<div className="ratings-drilldown">
  {viewLevel === 'yearly' && years.map(year => (
    <div className="card" onClick={() => { setSelectedYear(year); setViewLevel('quarterly'); }}>
      <h4>{year}</h4>
    </div>
  ))}

  {viewLevel === 'quarterly' && quarters.map(q => (
    <div className="card" onClick={() => { setSelectedQuarter(q); setViewLevel('monthly'); }}>
      <h4>{`Q${q.quarter} ${selectedYear}`}</h4>
    </div>
  ))}

  {viewLevel === 'monthly' && months.map(m => (
    <div className="card" onClick={() => { setSelectedMonth(m); setViewLevel('weekly'); }}>
      <h4>{m.label}</h4>
    </div>
  ))}

  {viewLevel === 'weekly' && weeks.map(w => (
    <div className="card" onClick={() => { setSelectedWeek(w); setViewLevel('daily'); }}>
      <h4>{w.label}</h4>
    </div>
  ))}

  {viewLevel === 'daily' && days.map(d => (
    <div className="card" onClick={() => { setSelectedDay(d); setViewLevel('table'); }}>
      <h4>{d.label}</h4>
    </div>
  ))}
  ...
  {viewLevel === 'table' && (
    <table>
      ...
      <tbody>
        {feedbackList
          .filter(f => isWithinDay(f.createdAt.toDate(), selectedDay.date))
          .map((f, i) => (
            <tr key={i}>
              <td>{f.email}</td>
              <td>{f.rating}</td>
                        ...
                        {viewLevel === 'table' && (
                            <div>
                                ...
                                <table>
                                    ...
                                    <tbody>
                                        {feedbackList
                                            .filter(f => f.createdAt?.toDate().toLocaleDateString().includes(selectedPeriod))
                                            .map((f, i) => (
                                                <tr key={i}>
                                                    <td>{f.email}</td>
                                                    <td>{f.rating}</td>
