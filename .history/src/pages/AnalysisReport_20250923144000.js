...
const AnalysisReport = () => {
    ...
    const getUserActivityData = useMemo(() => {
        ...
        filteredUsers.forEach(user => {
            const date = moment(user.registeredDate);
            let key = filter === 'Monthly' ? date.format('MMM') : filter === 'Quarterly' ? `Q${Math.ceil((date.month() + 1) / 3)}` : date.year().toString();
        ...
        const getKeys = () => {
            ...
            return [...new Set(filteredUsers.map(user => moment(user.registeredDate).year()))].sort().map(String);
        };
    ...
    const generateChartData = (keyFn, valueFn) => {
        const periods = filter === 'Monthly' ? moment.monthsShort() : filter === 'Quarterly' ? ['Q1', 'Q2', 'Q3', 'Q4'] : [...new Set(filteredUsers.map(u => moment(u.registeredDate).year()))].sort();
        const group = {};
        periods.forEach(p => group[p] = {});

        filteredUsers.forEach(u => {
            const periodKey = keyFn(u);
            const val = valueFn(u);
            group[periodKey][val] = (group[periodKey][val] || 0) + 1;
        });
    ...
    // Age Distribution grouped by decade, filtered
    const ageGroupData = useMemo(() => {
        // Determine the period key for each user
        const getPeriod = (date) => {
            if (filter === 'Monthly') return moment(date).format('MMM');
            if (filter === 'Quarterly') return `Q${Math.ceil((moment(date).month() + 1) / 3)}`;
            return moment(date).year().toString();
        };

        const ageCounts = {}; // { '0-9': {Q1: 3, Q2:0,...}, ... }

        filteredUsers.forEach(u => {
            if (u.age != null) {
                let rangeLabel = '';
                if (u.age >= 90) rangeLabel = '90+';
                else {
                    const rangeStart = Math.floor(u.age / 10) * 10;
                    rangeLabel = `${rangeStart}-${rangeStart + 9}`;
                }

                const periodKey = getPeriod(u.registeredDate);
                if (!ageCounts[rangeLabel]) ageCounts[rangeLabel] = {};
                ageCounts[rangeLabel][periodKey] = (ageCounts[rangeLabel][periodKey] || 0) + 1;
            }
        });

        // Only keep age groups with at least one entry
        const ageGroups = Object.entries(ageCounts)
            .filter(([_, periods]) => Object.values(periods).some(v => v > 0))
            .map(([ageGroup, periods]) => {
                ...
                periodKeys.forEach(pk => { if (!(pk in periods)) periods[pk] = 0; });

                return { ageGroup, ...periods };
            });
