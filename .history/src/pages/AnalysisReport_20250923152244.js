...
const AnalysisReport = () => {
    ...
    const getUserActivityData = useMemo(() => {
        ...
        // Map to array with totals
        const result = periodKeys.map(pk => ({
            name: pk,
        ...
        // Add total row
        const total = { name: 'Total', users: 0, months: {} };
        periodKeys.forEach(pk => {
            total.users += activity[pk].users;
            Object.entries(activity[pk].months).forEach(([m, count]) => {
                total.months[m] = (total.months[m] || 0) + count;
            });
        });
        result.push(total);
    ...
    // Custom Tooltip to handle totals
const CustomUserTooltip = ({ active, payload, label }) => {
    ...
    if (filter === 'Monthly') {
        breakdown = [`Users: ${data.users}`];
    } else if (filter === 'Quarterly' || filter === 'Yearly') {
    ...
    return (
        <div style={{ background: '#fff', border: '1px solid #ccc', padding: 10, borderRadius: 5 }}>
            ...
            {data.name !== 'Total' && <hr />}
            {data.name === 'Total' && <p><strong>Total Users: {data.users}</strong></p>}
        </div>
    );
};
