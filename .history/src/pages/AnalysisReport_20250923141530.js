...
const AnalysisReport = () => {
    ...
    const ageChartData = useMemo(() => {
        ...
        filteredUsers.forEach(u => {
            ...
            let ageKey = groupAge
                ? `${Math.floor(u.age / 10) * 10}-${Math.floor(u.age / 10) * 10 + 9}`
                : u.age.toString();

            group[periodKey][ageKey] = (group[periodKey][ageKey] || 0) + 1;
        });

        // Fill missing age groups with 0
        const allAges = groupAge
            ? [...new Set(filteredUsers.map(u => `${Math.floor(u.age / 10) * 10}-${Math.floor(u.age / 10) * 10 + 9}`))]
            : [...new Set(filteredUsers.map(u => u.age.toString()))];
    ...
    }, [filteredUsers, filter, groupAge]);


    const hierarchicalAgeData = useMemo(() => {
        // Prepare age keys
        const allAgeGroups = groupAge
            ? [...new Set(users.map(u => `${Math.floor(u.age / 10) * 10}-${Math.floor(u.age / 10) * 10 + 9}`))]
            : [...new Set(users.map(u => u.age.toString()))];

        const data = { Q1: {}, Q2: {}, Q3: {}, Q4: {} };

        filteredUsers.forEach(u => {
            const date = moment(u.registeredDate);
            const quarter = `Q${Math.ceil((date.month() + 1) / 3)}`;
            const month = date.format('MMM');
            const week = `W${date.week()}`;
            const day = date.format('ddd');

            const ageKey = groupAge
                ? `${Math.floor(u.age / 10) * 10}-${Math.floor(u.age / 10) * 10 + 9}`
                : u.age.toString();

            if (!data[quarter][month]) data[quarter][month] = {};
            if (!data[quarter][month][week]) data[quarter][month][week] = {};
            if (!data[quarter][month][week][day]) data[quarter][month][week][day] = {};
            data[quarter][month][week][day][ageKey] = (data[quarter][month][week][day][ageKey] || 0) + 1;
        });

        return data;
    }, [filteredUsers, groupAge]);
    const ageChartViewData = useMemo(() => {
        let levelData = hierarchicalAgeData;
        drillDownPath.forEach(p => {
            if (levelData && levelData[p]) levelData = levelData[p];
            else levelData = {};
        });

        const chartData = Object.entries(levelData).map(([key, value]) => {
            if (typeof value === 'number') return { ageGroup: key, value }; // leaf node
            else if (typeof value === 'object') {
                // sum all nested counts
                const sum = Object.values(value).reduce((s1, v1) => {
                    if (typeof v1 === 'number') return s1 + v1;
                    return s1 + Object.values(v1).reduce((s2, v2) => {
                        if (typeof v2 === 'number') return s2 + v2;
                        return s2 + Object.values(v2).reduce((s3, v3) => {
                            if (typeof v3 === 'number') return s3 + v3;
                            return s3 + Object.values(v3).reduce((s4, v4) => s4 + v4, 0);
                        }, 0);
                    }, 0);
                }, 0);
                return { ageGroup: key, value: sum };
            }
        }).filter(d => d.value > 0 || drillDownPath.length === 0); // show 0 only at top level

        return chartData;
    }, [hierarchicalAgeData, drillDownPath]);
    const handleBarClick = (data) => {
        setDrillDownPath(prev => [...prev, data.ageGroup]);
    };
        ...
        <div className="dashboard-wrapper">
            ...
            <main className="dashboard-main">
                ...
                <div style={{ marginBottom: '1rem' }}>
                    <button onClick={handleBackClick} disabled={drillDownPath.length === 0}>Back</button>
                </div>

                <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={ageChartViewData} onClick={handleBarClick}>
