...
const FeedbackReview = () => {
    ...
    const calculateStats = (data, tabType) => {
        ...
        if (tabType === 'Feature Feedback') {
            ...
            filtered = data.filter(item => item.feedbackType === 'Location Feedback' || item.feedbackType === 'App Feedback');
        }
        ...
        filtered.forEach(item => {
            if (item.feedbackType === 'Location Feedback') {
                if (item.location) {
                    if (!locationRatings[item.location]) locationRatings[item.location] = [];
                    locationRatings[item.location].push(item.rating || 0);
                }
            } else if (item.feedbackType === 'App Feedback') {
                if (item.feature) {
                    if (!featureRatings[item.feature]) featureRatings[item.feature] = [];
                    featureRatings[item.feature].push(item.rating || 0);
                }
            }
        });

        // Calculate averages
        const locationAvg = Object.entries(locationRatings).map(([key, ratings]) => ({
            key,
            avg: ratings.reduce((a, b) => a + b, 0) / ratings.length
        }));
        const featureAvg = Object.entries(featureRatings).map(([key, ratings]) => ({
            key,
            avg: ratings.reduce((a, b) => a + b, 0) / ratings.length
        }));

        if (locationAvg.length) {
            locationAvg.sort((a, b) => b.avg - a.avg);
            setMostLovedLocation(locationAvg[0].key);
            setAreaOfConcern(locationAvg[locationAvg.length - 1].key);
        }
        if (featureAvg.length) {
            featureAvg.sort((a, b) => b.avg - a.avg);
            setMostLovedFeature(featureAvg[0].key);
            setAreaOfConcernFeature(featureAvg[featureAvg.length - 1].key);
        }
    };
        ...
        <div className="dashboard-wrapper">
            ...
            <div className="dashboard-main">
                ...
                <div className="mtab-buttons">
                    <button
                        className={`mtab ${isLocationTab ? 'active' : ''}`}
                        onClick={() => handleTabClick('Location Feedback')}
                    >
                        Location Feedback
                    </button>
                    <button
                        className={`mtab ${isFeatureTab ? 'active' : ''}`}
                        onClick={() => handleTabClick('Feature Feedback')}
                    >
                        Feature Feedback
                    </button>
                    <button
                        className={`mtab ${isAllTab ? 'active' : ''}`}
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    setSelectedFeatureOrLocation(
                                                        entry.feedbackType === 'App Feedback' ? entry.feature || 'N/A' : entry.location || 'N/A'
                                                    );
