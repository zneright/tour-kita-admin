...
const FeedbackReview = () => {
    ...
    const avgLocation = locationFeedback.reduce((a, c) => a + (c.rating || 0), 0) / locationFeedback.length;
    ...
    const avgFeature = featureFeedback.reduce((a, c) => a + (c.rating || 0), 0) / featureFeedback.length;
    ...
    const calculateStats = (data, tabType) => {
        ...
        // 🔹 Calculate overall (from ALL feedback, not filtered)
        const globalRatings = {};
        data.forEach((item) => {
            const key = item.feature || item.location;
            if (key) {
