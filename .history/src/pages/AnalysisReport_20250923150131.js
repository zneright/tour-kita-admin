...
const AnalysisReport = () => {
    ...
    const getUserActivityData = useMemo(() => {
        ...
        filteredUsers.forEach(user => {
            ...
            else if (filter === 'Yearly') key = date.format('MMM D'); // e.g., Jan 9
            activity[key] = (activity[key] || 0) + 1;
        });

        const getKeys = () => {
          // Inside getUserActivityData
if (filter === 'Yearly') {
    // Aggregate per month-day (e.g., Jan 1, Jan 2)
    const activity = {};
    filteredUsers.forEach(u => {
        const date = moment(u.registeredDate);
        const key = date.format('MMM D'); // e.g., Jan 9
        activity[key] = (activity[key] || 0) + 1;
    });
    ...
    const days = [];
    for (let m = 0; m < 12; m++) {
        const daysInMonth = moment(`${selectedYear}-${m + 1}`, 'YYYY-M').daysInMonth();
        for (let d = 1; d <= daysInMonth; d++) {
            const label = moment(`${selectedYear}-${m + 1}-${d}`, 'YYYY-M-D').format('MMM D');
            days.push({ name: label, users: activity[label] || 0 });
        }
    }
    return days;
}
    ...
    const CustomAgeTooltip = ({ active, payload, label, filter }) => {
        ...
        return (
            <div className="custom-tooltip" style={{ background: '#fff', border: '1px solid #ccc', padding: 10, borderRadius: 5 }}>
                <p><strong>{filter === 'Yearly' ? `Date: ${label}` : `Period: ${label}`}</strong></p>

                {filter === 'Quarterly' && quarterMonths[label] && (
                    <p><em>Includes: {quarterMonths[label].join(', ')}</em></p>
                )}
    ...
    const ageGroupChartData = useMemo(() => {
        ...
        const data = periodKeys.map(pk => {
            ...
            filteredUsers.forEach(u => {
                const period = filter === 'Monthly' ? moment(u.registeredDate).format('MMM')
                    : filter === 'Quarterly' ? `Q${Math.ceil((moment(u.registeredDate).month() + 1) / 3)}`
                        : moment(u.registeredDate).year().toString();
                if (period === pk) {
                    const rangeStart = Math.floor(u.age / 10) * 10;
