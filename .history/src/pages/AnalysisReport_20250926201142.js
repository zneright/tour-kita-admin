...
const AnalysisReport = () => {
    ...
    const getFeedbackReport = (period = 'Monthly') => {
        const periods = period === 'Monthly' ? moment.monthsShort() :
            period === 'Quarterly' ? ['Q1', 'Q2', 'Q3', 'Q4'] :
                [...new Set(feedbacks.map(fb => moment(fb.createdAt).year()))].sort();

        const report = periods.map(p => {
            const fbInPeriod = feedbacks.filter(fb => {
                const date = moment(fb.createdAt);
                if (!date.isValid()) return false;

                if (period === 'Monthly') return date.format('MMM') === p;
                if (period === 'Quarterly') {
            ...
            const ratings = fbInPeriod.map(f => f.rating).filter(r => typeof r === 'number');
            const average = ratings.length ? (ratings.reduce((sum, r) => sum + r, 0) / ratings.length).toFixed(1) : 0;

            const topLocation = fbInPeriod.reduce((acc, f) => {
                if (!f.location) return acc;
                if (!acc[f.location]) acc[f.location] = [];
                acc[f.location].push(f.rating);
                return acc;
            }, {});

            const top = Object.entries(topLocation).map(([loc, arr]) => ({ loc, avg: (arr.reduce((s, r) => s + r, 0) / arr.length).toFixed(1) }))
                .sort((a, b) => b.avg - a.avg)
                .slice(0, 3);

            const low = Object.entries(topLocation).map(([loc, arr]) => ({ loc, avg: (arr.reduce((s, r) => s + r, 0) / arr.length).toFixed(1) }))
                .sort((a, b) => a.avg - b.avg)
                .slice(0, 3);

            return { period: p, count: fbInPeriod.length, average, top, low };
        });
    ...
    const getAreasOfConcern = () => {
        ...
        feedbacks.forEach(fb => {
            if (!fb.location || typeof fb.rating !== 'number') return;
            if (!locationMap[fb.location]) locationMap[fb.location] = [];
            locationMap[fb.location].push({ date: fb.createdAt, rating: fb.rating });
        });

        const concerns = [];
        for (const loc in locationMap) {
            const ratings = locationMap[loc].sort((a, b) => new Date(a.date) - new Date(b.date));
            let consecutive = 0;
            for (const r of ratings) {
                if (r.rating < 3) { // threshold for concern
                    consecutive++;
                    if (consecutive >= 2) concerns.push({ location: loc, consecutive });
                } else {
                    consecutive = 0;
                }
            }
    ...
    const CustomAgeTooltip = ({ active, payload, label, filter }) => {
        ...
        return (
            <div style={{ background: '#fff', border: '1px solid #ccc', padding: 10, borderRadius: 5 }}>
                ...
                <hr />
                <p><strong>Total Users: {data.total || 0}</strong></p>
            </div>
        );
    };
                    ...
                    <div className="chart-filters">
                        <div className="filter-group">
                            ...
                            <div style={{ textAlign: 'right', marginTop: 20 }}>
                                <button className="export-button" onClick={() => exportReport(feedbacks, users, filter, selectedYear)}>Export PDF/Excel</button>
                            </div>
