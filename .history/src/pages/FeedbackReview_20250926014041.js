...
const FeedbackReview = () => {
    ...
    // QUARTERS DRILLDOWN (same as yearly)
    ...
    // MONTHS DRILLDOWN (like quarters)
    ...
    const weeksForMonth = (year, monthIndex) => {
        ...
        return weekStarts.map(ws => {
            ...
            const end = we > monthEnd ? monthEnd : we;
    ...
    // DAYS DRILLDOWN (like weeks)
    const daysForWeek = (start, end) => {
        const days = eachDayOfInterval({ start, end }).map(d => {
            const dayStart = startOfDay(d);
            const dayEnd = endOfDay(d);
            const entries = entriesFor(dayStart, dayEnd);
            return {
                date: d,
