...
const CLOUDINARY_UPLOAD_PRESET = "react_ar_upload";
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const audioUrls = await Promise.all(
                formData.audioFiles.map(file => uploadToCloudinary(file))
            );

            const videoUrls = await Promise.all(
                formData.videoFiles.map(file => uploadToCloudinary(file))
            );
