...
const AnalysisReport = () => {
    ...
    const getUserActivityData = useMemo(() => {
        ...
        const getKeys = () => {
            ...
            if (filter === 'Yearly') {
                // Include Jan 1 → Dec 31 keys
                const days = [];
                for (let m = 0; m < 12; m++) {
                    const daysInMonth = moment(`${selectedYear}-${m + 1}`, 'YYYY-M').daysInMonth();
                    for (let d = 1; d <= daysInMonth; d++) {
                        days.push(moment(`${selectedYear}-${m + 1}-${d}`, 'YYYY-M-D').format('MMM D'));
                    }
                }
                return days;
            }
    ...
    const CustomAgeTooltip = ({ active, payload, label, filter }) => {
        ...
        return (
            <div className="custom-tooltip" style={{
                background: '#fff', border: '1px solid #ccc', padding: 10, borderRadius: 5
            }}>
                <p><strong>{filter === 'Yearly' ? `Month: ${label}` : `Period: ${label}`}</strong></p>
