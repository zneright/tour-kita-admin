...
const AnalysisReport = () => {
    ...
    const [drilldownLevel, setDrilldownLevel] = useState(filter); // Weekly, Monthly, Quarterly, Yearly
    const [drilldownData, setDrilldownData] = useState([]);
    const [breadcrumb, setBreadcrumb] = useState([]);

    const prepareChartData = (feedbacks, level, parentPeriod = null) => {
        const grouped = groupFeedbackByPeriod(feedbacks, level);
        return Object.entries(grouped).map(([period, items]) => {
            const avg =
                items.reduce((sum, fb) => sum + (fb.rating || 0), 0) / items.length;
            return {
                name: period,
                average: parseFloat(avg.toFixed(1)),
                count: items.length,
                items, // keep raw for drilldown
            };
        });
    };

    useEffect(() => {
        // Load initial drilldown
        const initial = prepareChartData(feedbacks, filter);
        setDrilldownData(initial);
        setDrilldownLevel(filter);
        setBreadcrumb([filter]);
    }, [feedbacks, filter]);

    const handleBarClick = (entry) => {
        let nextLevel = null;
        if (drilldownLevel === "Yearly") nextLevel = "Quarterly";
        else if (drilldownLevel === "Quarterly") nextLevel = "Monthly";
        else if (drilldownLevel === "Monthly") nextLevel = "Weekly";
        else if (drilldownLevel === "Weekly") nextLevel = "Daily"; // Mon–Sun

        if (!nextLevel) return;

        const nextData = prepareChartData(entry.items, nextLevel, entry.name);
    ...
    const handleBack = () => {
        // Pop breadcrumb and go up
        if (breadcrumb.length <= 1) return;
        const newBreadcrumb = breadcrumb.slice(0, -1);
        setBreadcrumb(newBreadcrumb);
        const level = newBreadcrumb[newBreadcrumb.length - 1];
        const initial = prepareChartData(feedbacks, level);
        setDrilldownData(initial);
    ...
    const groupByDay = (items) => {
        const grouped = {};
        items.forEach(fb => {
            const day = moment(fb.createdAt?.toDate?.() || fb.createdAt).format('dddd');
            if (!grouped[day]) grouped[day] = [];
            grouped[day].push(fb.rating);
        });

        return Object.entries(grouped).map(([day, ratings]) => ({
            name: day,
    ...
    const groupFeedbackByPeriod = (feedbacks, filter) => {
        const grouped = {};

        feedbacks.forEach(fb => {
            if (typeof fb.rating !== 'number') return;

            const date = moment(fb.createdAt?.toDate?.() || fb.createdAt);
            let key = '';
            ...
            if (filter === 'Weekly') {
                ...
                key = `${date.year()}-Q${Math.ceil((date.month() + 1) / 3)}`;
    ...
    const getTopFeedbackByPeriod = (feedbacks, typeKey, labelKey, filter) => {
        const grouped = groupFeedbackByPeriod(
            feedbacks.filter(f => f.feedbackType === typeKey),
            filter
        );
        ...
        const result = Object.entries(grouped).map(([period, items]) => {
            ...
            items.forEach(fb => {
                ...
                subGrouped[key].push(fb.rating);
            });

            const avgRatings = Object.entries(subGrouped).map(([name, ratings]) => ({
                name,
                average: ratings.reduce((a, b) => a + b, 0) / ratings.length,
                count: ratings.length,
                period
            ...
            // sort based on filter (top or low)
            return {
                period,
                top: [...avgRatings].sort((a, b) => b.average - a.average)[0],
                low: [...avgRatings].sort((a, b) => a.average - b.average)[0],
                all: avgRatings
            };
        });
            ...
            <main className="dashboard-main">
                ...
                <div className="chart-container">
                    ...
                    {loading ? (
                        ...
                        <>
                            <div className="chart-container">
                                <h3>{filter} Feedback Summary</h3>
                                {['Location Feedback', 'App Feedback'].map((typeKey) => {
                                    const data = getTopFeedbackByPeriod(feedbacks, typeKey, typeKey === 'Location Feedback' ? 'location' : 'feature', filter);
                                    ...
                                    return (
                                        <div key={typeKey}>
                                            ...
                                            {selectedPeriod ? (
                                                <>
                                                    <button onClick={() => setSelectedPeriod(null)}>⬅ Back</button>
                                                    <h5>{selectedPeriod.period}</h5>
                                                    {selectedPeriod.all.map((entry, i) => (
                                                        <div key={i} className="feedback-card">
                                                            <strong>{entry.name}</strong> — {entry.average.toFixed(1)}⭐ ({entry.count})
                                                        </div>
                                                    ))}
                                                </>
                                            ) : (
                                                data.map((entry, i) => (
                                                    ...
                                                    >
                                                        ...
                                                        Top: {entry.top?.name} ({entry.top?.average.toFixed(1)}⭐)<br />
                                                        Low: {entry.low?.name} ({entry.low?.average.toFixed(1)}⭐)
                                                    </div>
                                                ))
                                            )}
                                        </div>
