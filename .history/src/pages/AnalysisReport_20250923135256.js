...
const AnalysisReport = () => {
    ...
    const [groupGenderByPeriod, setGroupGenderByPeriod] = useState(true);
    ...
    const genderChartData = useMemo(() => {
        if (!groupGenderByPeriod) {
            // Total counts (not grouped)
            const counts = {};
            filteredUsers.forEach(u => {
            ...
            filteredUsers.forEach(u => {
                let key = '';
                if (filter === 'Monthly') key = moment(u.registeredDate).format('MMM');
                else if (filter === 'Quarterly') key = `Q${Math.ceil((moment(u.registeredDate).month() + 1) / 3)}`;
                else key = u.registeredDate ? moment(u.registeredDate).year() : 'Unknown';

                const gender = u.gender || 'Prefer Not to Say';
                if (!group[key]) group[key] = {};
                group[key][gender] = (group[key][gender] || 0) + 1;
            });

            return Object.entries(group)
                .sort(([a], [b]) => {
                    if (filter === 'Monthly') return moment(a, 'MMM').month() - moment(b, 'MMM').month();
                    if (filter === 'Quarterly') return a.localeCompare(b);
                    return a - b;
                })
                .map(([period, counts]) => ({ period, ...counts }));
        }
    }, [filteredUsers, filter, groupGenderByPeriod]);
    ...
    const userTypeChartData = useMemo(() => {
        ...
        filteredUsers.forEach(u => {
            ...
            if (!group[key]) group[key] = {};
            group[key][type] = (group[key][type] || 0) + 1;
        });
    ...
    const ageChartData = useMemo(() => {
        ...
        filteredUsers.forEach(u => {
            ...
            const age = u.age || 0;
            if (!group[key]) group[key] = {};
            group[key][age] = (group[key][age] || 0) + 1;
        });
                ...
                <div className="chart-container">
                    ...
                    <div style={{ marginBottom: '1rem' }}>
                        <label>
                            <input
                                type="checkbox"
                                checked={groupGenderByPeriod}
                                onChange={(e) => setGroupGenderByPeriod(e.target.checked)}
                            />{' '}
                            Group by {filter}
                        </label>
                    </div>
                    <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={genderChartData}>
                            <XAxis dataKey={groupGenderByPeriod ? "period" : "name"} />
                            <YAxis />
                            <Tooltip />
                            {Object.keys(genderChartData[0] || {})
                                .filter(k => groupGenderByPeriod ? k !== "period" : k !== "name")
                                .map((gender, idx) => (
                                    <Bar key={gender} dataKey={gender} stackId="a" fill={COLORS[idx % COLORS.length]} />
                                ))}
                        </BarChart>
