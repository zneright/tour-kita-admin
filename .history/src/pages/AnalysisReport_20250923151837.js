...
const AnalysisReport = () => {
   ...
   const getU serActivityData = useMemo(() => {
        const activity = {};
        ...
        periodKeys.forEach(pk => {
            activity[pk] = {
                users: 0,
                months: {} // for quarterly tooltip
            };
            if (filter === 'Quarterly') {
                const months = quarterMonths[pk] || [];
                months.forEach(m => activity[pk].months[m] = 0);
            }
        });
        ...
        filteredUsers.forEach(user => {
            ...
            if (filter === 'Monthly') {
                ...
                let quarter;
                if (monthIndex <= 2) quarter = 'Q1';
                else if (monthIndex <= 5) quarter = 'Q2';
                else if (monthIndex <= 8) quarter = 'Q3';
                else quarter = 'Q4';

                const monthName = date.format('MMM');
                activity[quarter].users += 1;
                activity[quarter].months[monthName] += 1; // store monthly count for tooltip
            } else {
