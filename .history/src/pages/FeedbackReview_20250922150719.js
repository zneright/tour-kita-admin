...
const FeedbackReview = () => {
    ...
    const calculateStats = (data, tabType) => {
        ...
        const getFilteredByPeriod = (list, period) => {
            const now = new Date();
            let start, end;
        ...
        const getRankedList = (data, isFeature) => {
            ...
            return Object.entries(categoryRatings)
                ...
                .sort((a, b) => b.avg - a.avg); // Top → lowest
        };
const [showAll, setShowAll] = useState(false);

const rankedList = getRankedList(filteredData, isFeatureTab);
const topList = showAll ? rankedList : rankedList.slice(0, 3);

return (
  <div>
    <h3>{period.charAt(0).toUpperCase() + period.slice(1)} Overview</h3>
    <ul>
      {topList.map(item => (
        <li key={item.key}>
          {item.key} - Avg: {item.avg.toFixed(1)} ⭐ ({item.count} feedbacks)
        </li>
      ))}
    </ul>
    {rankedList.length > 3 && (
