...
const AnalysisReport = () => {
    ...
    const prepareChartData = (feedbacks, level, parentPeriod = null) => {
        ...
        return Object.entries(grouped).map(([period, items]) => {
            // Group by location/app for this period
            const subGrouped = {};
            items.forEach(fb => {
                const key = fb.location || fb.feature || "N/A";
            ...
            const top = [...avgRatings].sort((a, b) => b.average - a.average)[0];
            const low = [...avgRatings].sort((a, b) => a.average - b.average)[0];

            return {
                name: period,
                average: parseFloat(
                    (items.reduce((sum, fb) => sum + (fb.rating || 0), 0) / items.length).toFixed(1)
                ),
                count: items.length,
                top,
                low,
            ...
            <main className="dashboard-main">
                ...
                <div className="chart-container">
                    ...
                    {loading ? (
                        ...
                        <>
                            <div className="chart-container">
                                <h3>Feedback Drill-Down ({breadcrumb.join(" > ")})</h3>
                                {breadcrumb.length > 1 && (
                                ...
                                <Tooltip
                                    formatter={(value, name, props) => [
                                        `${value} ⭐`,
                                        `Avg Rating - Top: ${props.payload.top?.name} (${props.payload.top?.average.toFixed(1)}) | Low: ${props.payload.low?.name} (${props.payload.low?.average.toFixed(1)})`
                                    ]}
                                />
