...
const FeedbackReview = () => {
    ...
    const averageForEntries = (entries) => {
        ...
        const filtered = entries.filter(e => {
            if (isFeatureTab) return e.feedbackType === "App Feedback";
            if (isLocationTab) return e.feedbackType === "Location Feedback";
            return true;
        });
        if (!filtered.length) return null;
        const sum = filtered.reduce((acc, cur) => acc + (cur.rating || 0), 0);
    ...
    const topAndBottomForEntries = (entries) => {
        ...
        const arr = avgPerKey(filtered).filter(e => e.count > 0);
                            ...
                            <>
                                ...
                                <div className="drill-row">
                                    {quartersForYear(selectedYear).map((qObj) => {
                                        const entries = qObj.entries; // all entries in this quarter
                                        const avgValue = averageForEntries(entries);
                            ...
                            <>
                                ...
                                <div className="drill-row">
                                    {monthsForQuarter(selectedYear, selectedQuarter).map((mObj) => {
                                        const entries = mObj.entries; // all entries in this month
                                        const avgValue = averageForEntries(entries);
                                        const { top, low } = topAndBottomForEntries(entries);
