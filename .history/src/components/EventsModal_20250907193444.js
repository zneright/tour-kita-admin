...
const EventModal = ({ event, onClose, onUpdate }) => {
    ...
    useEffect(() => {
        ...
        setFormData({
            ...event,
            // Ensure times are in 24-hour format for editing
                    ...
                    <div className="modal-content">
                        ...
                        <div className="event-info-cards">
                            <div className="info-card">
                                <FaCalendarAlt /> {formData.startDate} {formData.endDate ? `- ${formData.endDate}` : ""}
                            </div>
                            <div className="info-card">
                                <FaClock /> {formatTime12Hour(formData.eventStartTime)} - {formatTime12Hour(formData.eventEndTime)}
                            </div>
                            <div className="info-card">
                                <FaMapMarkerAlt /> {formData.customAddress || formData.address}
                            </div>
                        </div>
