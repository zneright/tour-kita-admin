...
const ExportButtons = ({ users }) => {
    ...
    const handleExportPDF = async () => {
        ...
        let page = pdfDoc.addPage([600, 800]); // <-- use let here
        ...
        users.forEach((user, idx) => {
            ...
            y -= 15;
            if (y < 50) {
                page = pdfDoc.addPage([600, 800]); // now allowed
                y = 770;
            }
        });

        const pdfBytes = await pdfDoc.save(); // note: pdf-lib cannot encrypt client-side
