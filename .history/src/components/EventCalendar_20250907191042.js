...
const EventCalendar = ({ onDateSelect }) => {
    ...
    const fetchEvents = async () => {
        ...
        events.forEach(event => {
            if (event.recurrence) {
                const start = new Date(event.recurrence.startDate);
                const end = new Date(event.recurrence.endDate);
                const days = event.recurrence.daysOfWeek.map(d => d.toLowerCase());

                for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
                    const dayStr = daysOfWeek[d.getDay()].toLowerCase();
                ...
                const key = formatLocalDate(new Date(event.startDate || event.date));
                if (!eventsByDate[key]) eventsByDate[key] = [];
                eventsByDate[key].push(event);
            }
        });
