 ...
 */
const generateFeedbackInsights = (feedbacks, groupKey, prevFeedbacks = []) => {
    const grouped = {};
    const prevGrouped = {};

    feedbacks.forEach(fb => {
        const key = fb[groupKey] || 'Unknown';
        if (!grouped[key]) grouped[key] = [];
        grouped[key].push(Number(fb.rating) || 0);
    });

    prevFeedbacks.forEach(fb => {
        const key = fb[groupKey] || 'Unknown';
        if (!prevGrouped[key]) prevGrouped[key] = [];
        prevGrouped[key].push(Number(fb.rating) || 0);
    });

    const insights = Object.entries(grouped).map(([key, ratings]) => {
        const prevRatings = prevGrouped[key] || [];
        ...
        const neutral = ratings.filter(r => r >= 3 && r < 4).length;
        const bad = ratings.filter(r => r < 3).length;

        const prevAvg = prevRatings.length
            ? prevRatings.reduce((a, b) => a + b, 0) / prevRatings.length
            : 0;
        const trend = prevAvg ? ((avg - prevAvg) / prevAvg * 100).toFixed(1) : 'N/A';

        return { key, count, avg: avg.toFixed(1), top, low, good, neutral, bad, trend };
    });
