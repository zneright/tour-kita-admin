        ...
        <div className="ratings-drilldown">
            ...
            {viewLevel === "table" && selectedDay && (
                <>
                    ...
                    <FeedbackTable
                        loading={false} // or true if fetching asynchronously
                        feedback={entriesFor(startOfDay(selectedDay), endOfDay(selectedDay))}
                        isAllTab={false}
                        isFeatureTab={isFeatureTab}
                        renderStars={(rating) => "★".repeat(rating) + "☆".repeat(5 - rating)} // example
                        formatTimestamp={(ts) => format(asDate(ts), "PPpp")}
                        onImageClick={(url) => console.log("Image clicked:", url)}
                        onSendMessage={(email, featureOrLocation) =>
                            console.log("Send message to:", email, featureOrLocation)
                        }
                    />
                </>
            )}
